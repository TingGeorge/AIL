...
export function App(){
 ...
 async function startSearch(){
  ...
  setSearchStarted(true);setSearchState(initialSearch());setSearchError("");setSearching(true);go("search");const location=position;setPosition(null);
  try{await api.search({need:{...valid.data,soft_preferences:[...new Set([...valid.data.soft_preferences,...account.data.settings.prefs])]},exclude:[...new Set([...account.data.settings.exclude,...valid.data.exclude_tags])],location},event=>{if(id!==requestId.current||controller.signal.aborted)return;setSearchState(state=>reduceSearch(state,event));if("records" in event)mergeRecords(event.records);if("step" in event&&event.step==="done")mergeRecords([...event.pending,...event.excluded]);},controller.signal);if(id===requestId.current)navigate("results");}
 ...
 {current.view!=="welcome"&&<header className="app-header glass"><button className="icon-button" aria-label={current.view==="home"?"回到歡迎頁":"返回首頁"} onClick={()=>go(current.view==="home"?"welcome":"home")}><ArrowLeft aria-hidden="true"/></button><div className="header-title"><b>{titles[current.view]}</b><span><MapPin aria-hidden="true"/>{current.view==="offers"?"適用地區與資格依各筆優惠":"主要涵蓋圓山，延伸至士林"}</span></div><button className="icon-button" aria-label="帳號與登入" onClick={()=>go("account")}><UserRound aria-hidden="true"/></button></header>}
 ...
 <div className="location-panel"><MapPin aria-hidden="true"/><div><b>{position?"已取得本次搜尋位置":"要比較距離，請先同意定位"}</b><p>位置只用於這次搜尋，不會儲存。拒絕定位仍可搜尋，但距離未知，不代表符合距離限制。</p></div><button className="secondary-action" disabled={locating} onClick={position?()=>setPosition(null):locate}>{locating?"定位中…":position?"清除本次位置":"只在這次搜尋使用定位"}</button></div>
 ...
 {current.view==="results"&&!searchStarted&&<BrowseResults list={account.data.list} onList={id=>void toggle(id,"list")} onOpen={id=>go("detail",id)} onAdjust={openManual}/>}
 {current.view==="results"&&searchStarted&&<div className="results-page"><div className="result-notices">{[...results,...searchState.pending,...searchState.excluded].some(isDemoRecord)&&<p className="notice warning"><b>目前包含專案示範資料</b>價格、地址、來源與優惠僅供測試串接，不是已驗證的真實商家資訊，請勿據此購買或前往。</p>}{searchError&&<div role="alert" className="notice error">{searchError}<button type="button" onClick={()=>go("filters")}>回到條件重試</button></div>}{!searchState.complete&&!searchError&&<p className="notice">搜尋尚未完成，目前僅顯示已收到的結果。可回到條件重新搜尋。</p>}</div><ResultsView preferredCategories={need.target_categories} records={results} pending={searchState.pending} excluded={searchState.excluded} list={account.data.list} survival={account.data.settings.survival} onList={id=>void toggle(id,"list")} onOpen={id=>go("detail",id)} onAdjust={()=>go("filters")}/></div>}
