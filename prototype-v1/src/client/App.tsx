...
export function App(){
 ...
 {current.view==="offers"&&<section className="screen team-screen"><h1>團購優惠</h1><p className="section-copy">來源提供的優惠；不提供組團或代訂。</p>{results.filter(r=>r.group_offer).map(item=><button key={item.id} className="quick-team" onClick={()=>go("detail",item.id)}><Users/><span><b>{item.title}</b><small>{item.group_offer!.min_people} 人門檻 · 查看完整來源條件</small></span><ArrowRight/></button>)}{!results.some(r=>r.group_offer)&&<div className="empty-state"><Users/><h2>目前沒有已搜尋到的團購優惠</h2><p>先探索生活選項，有來源提供的優惠才會顯示。</p><button className="secondary-action" onClick={()=>go("home")}>回到探索</button></div>}</section>}
