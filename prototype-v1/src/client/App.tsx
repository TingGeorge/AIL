...
import { ResultsView, DetailActions, DetailView } from "./ResultsView.tsx";
...
const modes={daily:{short:"日常",title:"精打細算"},zero:{short:"零元",title:"零元探索"}};
...
function AiliMascot(){
 return <div className="aili-mascot" aria-hidden="true"><span className="mascot-orbit"/><svg viewBox="0 0 180 180" focusable="false"><path className="mascot-tail" d="M131 124c31 1 35-24 17-35"/><path className="mascot-ear" d="m53 52 5-29 23 20m46 9-5-29-23 20"/><rect className="mascot-head" x="42" y="39" width="96" height="82" rx="30"/><rect className="mascot-face" x="53" y="50" width="74" height="58" rx="22"/><circle className="mascot-eye mascot-eye-left" cx="76" cy="77" r="7"/><circle className="mascot-eye mascot-eye-right" cx="105" cy="77" r="7"/><path className="mascot-mouth" d="M82 94q9 8 18 0"/><path className="mascot-body" d="M62 116h56l12 42H50z"/><rect className="mascot-badge" x="78" y="126" width="24" height="19" rx="7"/><path className="mascot-badge-mark" d="m85 139 4-8h6l-4 8z"/><circle className="mascot-signal" cx="90" cy="22" r="5"/></svg></div>;
}
export function App(){
 ...
 async function toggle(id:string,kind:"favs"|"list"){
  ...
  try{await account.update(data=>({...data,[kind]:data[kind].includes(id)?data[kind].filter(v=>v!==id):[...data[kind],id]}));setMessage(canStore?(adding?(kind==="favs"?"已收藏並儲存到帳號":"已加入清單並儲存到帳號"):"已從帳號移除"):(adding?(kind==="favs"?"已收藏在此瀏覽器，登入後可同步":"已加入此瀏覽器清單，登入後可同步"):"已從此瀏覽器移除"));}catch{/* Account state displays retryable sync failure. */}
 }
 ...
 {current.view==="welcome"&&<section className="welcome-screen"><div className="welcome-topline"><span>OUR NORTH STAR</span><AiliMascot/></div><div className="welcome-mark"><span>ALL</span><span>IN</span><span>LIFE</span></div><p className="welcome-story"><span>ALL IN 不是走投無路。</span><strong>是有夢想，所以選擇全力以赴。</strong></p><div className="welcome-typewriter" aria-label="省下日常，投資未來。把每一筆節省，變成更接近夢想的一步。"><span className="type-line type-line-one" aria-hidden="true">省下日常，投資未來。</span><span className="type-line type-line-two" aria-hidden="true">把每一筆節省，變成更接近夢想的一步。</span></div><div className="welcome-visual" aria-hidden="true"><div className="money-rain">{Array.from({length:10},(_,index)=><span key={index}>NT$</span>)}</div><div className="welcome-compass"><i/><i/><i/><Compass/></div><b className="visual-caption">把選擇轉向真正重要的事</b></div><button className="primary-action" onClick={()=>go("home")}><Gift aria-hidden="true"/>立即探索生活選項<ArrowRight aria-hidden="true"/></button><button className="secondary-action" onClick={()=>go("account")}><LogIn aria-hidden="true"/>登入後同步清單</button><small>不登入也能搜尋；登入後才能跨裝置同步清單與收藏。<br/>語音、逐字稿與搜尋條件不會寫入帳號。</small></section>}
 ...
 <div className="mode-labels" role="group" aria-label="探索方式"><span className="mode-label-caption">探索</span><button type="button" className={`mode-label daily ${mode==="daily"?"active":""}`} aria-pressed={mode==="daily"} onClick={()=>chooseMode("daily")}><CircleDollarSign aria-hidden="true"/>日常</button><button type="button" className={`mode-label zero ${mode==="zero"?"active":""}`} aria-pressed={mode==="zero"} onClick={()=>chooseMode("zero")}><Gift aria-hidden="true"/>零元</button><button type="button" className="mode-label team" onClick={()=>go("offers")}><TicketPercent aria-hidden="true"/>團購<ArrowRight aria-hidden="true"/></button></div>
 ...
 <CatalogNotice/>
 <p className="home-footnote"><Info aria-hidden="true"/>目前顯示的是已整理的公開資料，不是即時網路搜尋；價格、名額與服務狀態請以來源最新資訊為準。</p>
 </section>}
