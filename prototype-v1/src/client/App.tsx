...
export function App(){
 ...
 async function startSearch(){
  ...
  setSearchState(initialSearch());setSearchError("");setSearching(true);go("search");const location=position;setPosition(null);
 ...
 {account.error&&<div className="status-banner error" role="alert">{account.error}{canStore&&<button onClick={()=>void account.update(data=>({...data})).catch(()=>{})}>重試同步</button>}<button onClick={()=>{if(confirm("重新載入會捨棄尚未同步的本機變更，是否繼續？"))void account.reload().catch(account.reject);}}>重新載入帳號資料</button><button aria-label="關閉帳號訊息" onClick={()=>account.setError("")}><X/></button></div>}
 ...
 {current.view==="offers"&&<section className="screen team-screen"><h1>團購優惠</h1>{results.some(r=>r.group_offer&&isDemoRecord(r))&&<p className="notice warning">示範測試優惠，不可據此購買、兌換或前往。</p>}<p className="section-copy">來源提供的優惠；不提供組團或代訂。</p>{results.filter(r=>r.group_offer).map(item=><button key={item.id} className="quick-team" onClick={()=>go("detail",item.id)}><Users/><span><b>{item.title}</b><small>{item.group_offer!.min_people} 人門檻 · 查看完整來源條件</small></span><ArrowRight/></button>)}{!results.some(r=>r.group_offer)&&<div className="empty-state"><Users/><h2>目前沒有已搜尋到的團購優惠</h2><p>先探索生活選項，有來源提供的優惠才會顯示。</p><button className="secondary-action" onClick={()=>go("home")}>回到探索</button></div>}</section>}
