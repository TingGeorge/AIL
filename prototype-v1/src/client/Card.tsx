import { comparableTotal, money, saving, statusPip, type Rec } from "../shared/records.ts";
  ...
  return (
    <article className="card rise" onClick={() => go(`/card/${r.id}`)} role="link" tabIndex={0}
      ...
      <div className="card-head">
        ...
        <div className="price">
          {total === 0 ? <span className="freetag">免費</span> : <span className="num">{total === null ? "目前資料不足，無法估算總費用" : money(total)}</span>}
      ...
      <p className="status-line">
        ...
        {displayDataStatus(r.data_status)} · {r.source_type === "curated" ? "人工整理" : "網路搜尋"} · {r.verified_at}
      ...
      <div className="tagsrow">
        ...
        {reportCount > 0 && <span className="warntag"><span className="pip red" aria-hidden="true" />{reportCount} 則體驗回報 · 30 天內</span>}
      </div>
      {sv && sv.amount > 0 && <p className="saving">比 {r.baseline!.name} 省 NT${sv.amount}（{sv.pct}%）</p>}
      {sv && sv.amount <= 0 && <p className="note">比基準貴 NT${-sv.amount}</p>}
      {!sv && total !== null && total > 0 && <p className="note">目前資料不足，無法計算可節省金額</p>}
    </article>
  );
}
