...
export function NeedEditor({need,onChange}:{need:Need;onChange:(next:Need)=>void}){
 ...
 return <div className="need-editor">
  ...
  <fieldset className="tag-picker"><legend>類別（不選＝全部）</legend><div>{CATEGORIES.map(c=><button type="button" key={c} className={need.target_categories.includes(c)?"active":""} onClick={()=>set("target_categories",need.target_categories.includes(c)?need.target_categories.filter(v=>v!==c):[...need.target_categories,c])}>{c}</button>)}</div></fieldset>
  ...
  <fieldset className="tag-picker"><legend>排除成分／標籤</legend><div>{TAGS.map(t=><button type="button" key={t} className={need.exclude_tags.includes(t)?"active":""} onClick={()=>set("exclude_tags",need.exclude_tags.includes(t)?need.exclude_tags.filter(v=>v!==t):[...need.exclude_tags,t])}>{t}</button>)}</div></fieldset>
  ...
  <p className="fine-print">空白表示未知，並非 0。人數、時段、資格與未標示成分無法自動保證符合；有過敏需求請向提供者確認。</p>
 </div>;
}
