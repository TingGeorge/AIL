...
import { displayTagLabel } from "./display.ts";
...
export function NeedEditor({need,onChange}:{need:Need;onChange:(next:Need)=>void}){
 ...
 return <div className="need-editor">
  ...
  <details className="detail-disclosure need-extra"><summary><span>更多條件</span><small>{extraCount?`${extraCount} 項已設定`:"距離、偏好、資格"}</small><ChevronDown aria-hidden="true"/></summary><div className="disclosure-body"><div className="field-grid">
   <label className="field">最遠距離（公里）<input type="number" min="0" max="1000" step="0.1" value={need.max_distance_km??""} onChange={e=>set("max_distance_km",number(e.target.value))} placeholder="未指定"/></label>
   <label className="field">最長步行時間（分鐘）<input type="number" min="0" max="10000" value={need.max_minutes??""} onChange={e=>set("max_minutes",number(e.target.value))} placeholder="未指定"/></label>
  </div>
  <label className="field">可以接受需要先登記的選項嗎？<select value={need.registration_ok===null?"unknown":String(need.registration_ok)} onChange={e=>set("registration_ok",e.target.value==="unknown"?null:e.target.value==="true")}><option value="unknown">不限</option><option value="true">可以</option><option value="false">不要</option></select></label>
  <fieldset className="tag-picker"><legend>排除成分／標籤</legend><div>{TAGS.map(t=><button type="button" key={t} aria-pressed={need.exclude_tags.includes(t)} className={need.exclude_tags.includes(t)?"active":""} onClick={()=>set("exclude_tags",need.exclude_tags.includes(t)?need.exclude_tags.filter(v=>v!==t):[...need.exclude_tags,t])}>{displayTagLabel(t)}</button>)}</div></fieldset>
