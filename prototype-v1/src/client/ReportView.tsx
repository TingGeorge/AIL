...
export function ReportView({id,token,onLogin,onExpired}:{id:string;token:string|null;onLogin:()=>void;onExpired:(e:unknown)=>void}){
 ...
 return <details className="reports-panel detail-disclosure" id="candidate-reports"><summary><span><Flag aria-hidden="true"/>回報與留言</span><small>{loading?"讀取中":`${items.length} 則`}</small><ChevronDown aria-hidden="true"/></summary><div className="disclosure-body"><p className="fine-print">回報為使用者陳述，不代表已證實；公開顯示你的暱稱。請勿填寫個資。</p>
