...
import { bucket, comparableTotal, hardViolations, money, recentExperience, type Category, type Rec, type Report } from "../../shared/records.ts";
...
export function Results({ need, records, category, setNeed, listCount, chips, exclude, survival, reports }: {
  ...
  return (
    <Shell surface="white" title="確認" listCount={listCount} survival={survival}>
      <section className="rise">
        <p className="eyebrow">03 / 03 · 結果{survival && <span className="modechip">生存模式</span>}</p>
        <button className="chips" onClick={() => go("/confirm")} aria-label="編輯需求與限制">
          {chips.map((c) => <span key={c} className="chip">{c}</span>)}
          {exclude.map((t) => <span key={t} className="chip red">不吃{t}</span>)}
        </button>
        <p className="status-line">
          <span className={`pip ${paidOk ? "yellow" : "red"}`} /> CP值獵人 {paidOk ? "完成" : "未接上"}
          <span className={`pip ${freeOk ? "yellow" : "red"}`} /> 白嫖獵人 {freeOk ? "完成" : "未接上"}
      ...
      <nav className="tabs rise" aria-label="類別">
        {CATEGORIES.map((c) => (
          <button key={c} className={`tab ${c === category ? "on" : ""} ${need.target_categories.includes(c) ? "rel" : ""}`} onClick={() => go(`/results/${c}`)} aria-current={c === category ? "page" : undefined}>
            {c}<b>{counts[c]}</b>
          </button>
        ))}
      </nav>
      ...
      <section className="rise">
        <div className="cat">
          ...
          <p>{b.main.length} 筆通過限制與證據閘門 · {survival ? "免費優先，再" : ""}依總可比成本由低至高</p>
        </div>
        {b.main.length === 0 && (
          <div className="empty">
            <p className="lead">{inCat.length === 0 ? "這一類沒有找到資料。" : b.excluded.length ? "有資料，但都被硬限制排除。" : "有資料，但都還沒有可讀證據。"}</p>
            {byTag > 0 && <p className="note">不吃 {exclude.join("、")} 排除了 {byTag} 筆。到設定可以調整。</p>}
            {b.excluded.length > byTag && need.budget_total_twd !== null && cheapestExcluded !== undefined && (
              <>
                <p className="note">預算 {money(need.budget_total_twd)} 排除了 {b.excluded.length - byTag} 筆。</p>
                <button className="secondary" onClick={() => setNeed({ ...need, budget_total_twd: cheapestExcluded })}>放寬預算到 {money(cheapestExcluded)}</button>
              </>
            )}
            {need.free_only && b.excluded.length > 0 && <button className="secondary" onClick={() => setNeed({ ...need, free_only: false })}>不限免費</button>}
            {need.registration_ok === false && excludedBy("registration") > 0 && (
              <>
                <p className="note">「不想先登記」排除了 {excludedBy("registration")} 筆。</p>
      ...
      {b.pending.length > 0 && (
        <details className="rise fold">
          ...
          <p className={`note ${survival ? "acid" : ""}`}>價格、費用或有效期缺證據、過期或來源衝突。可當線索，不當推薦。{survival && " 生存模式下更要小心：便宜但錯的選項代價最高。"}</p>
          {b.pending.map(card)}
        </details>
      )}
    </Shell>
