...
import { COSTCO_MEMBER, filterStage, passesGate, type Rec } from "../src/shared/records.ts";
...
const stage = (recs: Rec[], n = need(), exclude: string[] = [], costcoOk = false) =>
  filterStage(recs, n, exclude, costcoOk, NOW);
...
test("filterStage：六個硬限制各自計數（FR-13 要說得出是哪一個）", () => {
  const recs = [
    ...
    rec({ id: "會員", eligibility: [COSTCO_MEMBER] }),
  ];
...
// SPEC-ingestion §8：沒有會員就買不到，不該出現在推薦裡。
test("filterStage：costco_ok 關閉時排除需要會員的候選，開啟時留著", () => {
  const recs = [rec({ id: "會員價", eligibility: [COSTCO_MEMBER] }), rec({ id: "一般" })];
  expect(stage(recs, need(), [], false).main.map((r) => r.id)).toEqual(["一般"]);
  expect(stage(recs, need(), [], true).main.map((r) => r.id).sort()).toEqual(["一般", "會員價"]);
});
