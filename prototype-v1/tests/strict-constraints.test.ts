...
test("known beef is excluded and unlabelled ingredients are pending, never main", () => {
  ...
  const result=filterStage([beef,unknown],{...EMPTY_NEED,exclude_tags:["牛"]},["牛"]);
...
test("dinner does not inherit a store's 24H opening when product is breakfast only", () => {
  ...
  const result=filterStage([breakfast,allDay],{...EMPTY_NEED,need:"晚餐",time_window:"18:00–20:00"},[]);
...
test("smaller food portions remain recommendations without claiming to feed the whole group", () => {
  const two={...food("two","雙人餐"),quantity_or_servings:"2個漢堡；每人各1個"};
  const unknown=food("unknown","點心");
  const result=filterStage([two,unknown],{...EMPTY_NEED,people_or_servings:3},[]);
  expect(result.main.map(r=>r.id)).toEqual(["two"]);
...
test("event dates and eligibility are strict without confusing food exclusions with transit", () => {
  ...
  const result=filterStage([event,uncertain],{...EMPTY_NEED,date:"2026-09-05"},[]);
  ...
  expect(filterStage([transit],{...EMPTY_NEED,exclude_tags:["牛"]},["牛"]).main).toHaveLength(1);
});
