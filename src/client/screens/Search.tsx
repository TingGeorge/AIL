...
import { passesGate } from "../records.ts";
...
export function Search({ preview, records, onDone, listCount }: { preview: boolean; records: Rec[]; onDone: (r: Rec[]) => void; listCount: number }) {
  const [paid, setPaid] = useState<AgentState>(init);
  const [free, setFree] = useState<AgentState>(init);

  useEffect(() => {
    if (!preview) {
      // Real search is not wired yet: report honestly, no fake success.
      const t = setTimeout(() => {
    ...
    // Preview: simulate two parallel agents with different pacing.
    const run = (agent: "paid" | "free", set: (s: AgentState) => void, pace: number) => {
      ...
      STEPS.forEach((_, i) => {
        timers.push(window.setTimeout(() => {
          ...
          const passed = i === STEPS.length - 1 ? mine.filter(passesGate).length : 0;
          set({ step: i, found, passed, done: i === STEPS.length - 1, failed: null });
        }, pace * (i + 1)));
      });
      return timers;
    };
  ...
  const panel = (name: string, zh: string, s: AgentState) => (
    <div className={`agent ${s.done ? "done" : ""} ${s.failed ? "failed" : ""}`}>
      <div className="agent-head">
        <span className="eyebrow"><span className={`pip ${s.failed ? "red" : s.done ? "yellow" : "blue"} ${s.done || s.failed ? "" : "live"}`} />{name}</span>
  ...
  return (
    <Shell surface="black" title="確認" listCount={listCount}>
      <section className="rise">
        <p className="eyebrow">02.5 / 03 · 搜尋中</p>
        <h1 className="display"><span className="outline">兩個</span><br /><span className="fill">Agent</span></h1>
        <p className="lead">並行搜尋。回來的資料還要過限制與證據檢查。</p>
      </section>
      <section className="rise agents">
        {panel("PAID AGENT", "付費選項搜尋", paid)}
        {panel("FREE AGENT", "免費資源搜尋", free)}
      </section>
      <p className="note rise">先套硬限制，再依總可比成本排序。無證據不排序。</p>
    </Shell>
  );
}
